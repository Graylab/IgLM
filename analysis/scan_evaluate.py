...
from iglm.model.tokens import SPECIES_TO_TOKEN, CHAIN_TO_TOKEN
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Test model predictions with alanine scans')
    parser.add_argument('paired_csv_file',
                        type=str,
                        help='Paired sequence csv')
    ...
    parser.add_argument('--output_dir',
                        type=str,
                        default=f'output_dir/scan_evaluate_{now}')
    ...
    model = transformers.GPT2LMHeadModel.from_pretrained(
        args.chkpt_dir).to(device)
    model.eval()

    vocab_file = './vocab.txt'
    tokenizer = transformers.BertTokenizerFast(vocab_file=vocab_file,
                                               do_lower_case=False)
    ...
    with h5py.File(f'{args.output_dir}/losses.h5', 'w') as h5_out:
        ...
        for _, row in tqdm(paired_dataset.iterrows(),
                           total=paired_dataset.shape[0]):
            for chain in ['Heavy', 'Light']:
                batch = build_scan_batch(row, chain, tokenizer, args.mask_len)

                with torch.no_grad():
                    batch['input_ids'], batch['labels'] = batch[
                        'input_ids'].to(device), batch['labels'].to(device)
                    outputs = model(batch['input_ids'], labels=batch['labels'])
                    loss_per_sample_in_batch = compute_loss_per_sample(
                        outputs['logits'],
                        batch['labels']).cpu().detach().numpy()
                    pos_per_sample_in_batch = np.arange(
                        loss_per_sample_in_batch.shape[0]
                    )  # positions are ordered within each batch since each sample is a position
                    chain_to_loss_per_sample[chain].append(
                        loss_per_sample_in_batch)
                    chain_to_pos_per_sample[chain].append(
                        pos_per_sample_in_batch)
        ...
        for chain in ['Heavy', 'Light']:
            ...
            h5_out.create_dataset(f'{chain}_num_samples_per_pos',
                                  data=num_samples_at_each_pos)
...
def build_scan_batch(row, chain, tokenizer, mask_len):
    ...
    seqs = []
    chain_token, species_token = CHAIN_TO_TOKEN[chain], SPECIES_TO_TOKEN[
        'human']
    for start in range(0, len(seq) - mask_len):
        end = start + mask_len
        seq_i = tokenizer.convert_tokens_to_ids([chain_token, species_token] +
                                                seq[:start] +
