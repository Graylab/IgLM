...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate t-SNE plot of model input embeddings.')
    ...
    parser.add_argument('--vocab_file',
                        type=str,
                        default='./vocab.txt',
                        help='Vocabulary file for model')
    parser.add_argument('--output_dir',
                        type=str,
                        default=f'output_dir/embeddings_{now}')
    ...
    aa_emb_wts = embeddings.weight[5:25].detach().cpu().numpy(
    )  # skip special tokens
    aa_labels = tokenizer.decode(np.arange(len(
        embeddings.weight))).split(' ')[5:25]

    # Plot projections and save to output directory
    axis_label_dict = {'tsne': 't-SNE', 'pca': 'PCA'}
    ...
    for projection in ['tsne', 'pca']:
        ...
        sabdab_res_df = get_sabdab_residue_dataframe(aa_emb_wts, aa_labels,
                                                     projection, x_label,
                                                     y_label)
        ...
        ax.legend(handles=handles[1:num_hydropathy + 1],
                  labels=labels[1:num_hydropathy + 1],
        ...
        ax.set_yticks([])
        plt.tight_layout()
        plt.savefig(f'{args.output_dir}/embeddings_{projection}.pdf',
                    dpi=600,
                    transparent=True)
        plt.show()


def get_sabdab_residue_dataframe(aa_emb_wts,
                                 aa_labels,
                                 projection,
                                 x_label,
                                 y_label,
    ...
    sabdab_res_df = pd.DataFrame(sabdab_res_data,
                                 columns=["Residue", x_label, y_label])
